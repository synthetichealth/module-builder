import { extractName, extractStates, extractRemarks } from './Module';
...
it('extracts the name from a module json', () => {
  expect(extractName(moduleJSON)).toEqual("Examplitis");
})

it('extracts the remarks from a module json', () => {
  let rem = extractRemarks(moduleJSON);
  expect(rem.length).toEqual(2);
})

it('extracts states from a module json', () => {
  let states = extractStates(moduleJSON);
  expect(states.length).toEqual(13)
})
