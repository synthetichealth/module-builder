...
class ModuleGraph extends Component<Props> {
  ...
  writeSVG(module){
    console.log(this.generateDOT(module))
  ...
  render() {
    ...
    return (
        <div ref={(mount) => { this.mount = mount }} className="Graph"></div>
    )
  }
  ...
  generateDOT(module){
    ...
    output += Object.keys(module.states).map( name => {
      ...
      return `${name} [id = "${node['id']}", shape = "${node['shape']}", style = "${node['style']}", label = "${node['label']}", fontcolor = "${node['fontcolor']};"]` 
